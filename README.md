...
mermaid will not draw.

Point it at anything else, or at a diagram whose type changed between the two versions, and it
shows both versions side by side instead, with a line saying why. That decision is re-made as you
type, so turning a diagram back into a flowchart brings the merged view back without touching the
picker.
